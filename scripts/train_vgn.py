...
from trg.networks import get_network
...
def main(args):
    ...
    trainer = create_trainer(net, optimizer, loss_fn_new, metrics, device)

    evaluator = create_evaluator(net, loss_fn_new, metrics, device)
    ...
    def log_validation_results(engine):
        ...
        val_writer.add_scalar("ECE", metrics["ECE"], epoch)
...
def loss_fn_new(y_pred, y, index):
    ...
    loss_rot = _rot_loss_fn(rotation_pred, real_rotations)
    loss_width = _width_loss_fn(width_pred, real_width)
    loss = loss_qual + real_label * (loss_rot + 0.01 * loss_width)
